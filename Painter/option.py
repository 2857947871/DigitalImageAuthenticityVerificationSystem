...
parser.add_argument('--pre_train', type=str, default='D:/DeepLearning/Image Manipulation Detection/code/AOT-GAN-for-Inpainting-master/experiments/G0000000.pt',
                    help='path to pretrained models')
parser.add_argument('--outputs', type=str, default='D:/DeepLearning/Image Manipulation Detection/code/AOT-GAN-for-Inpainting-master/outputs', 
...
# ----------------------------------
painter_args = parser.parse_args()
painter_args.iterations = int(painter_args.iterations)

painter_args.rates = list(map(int, list(painter_args.rates.split('+'))))

losses = list(painter_args.rec_loss.split('+'))
painter_args.rec_loss = {}
for l in losses: 
    weight, name = l.split('*')
    painter_args.rec_loss[name] = float(weight)